...
class Activity():
    ...
    def __init__(self, id: int, project_path: str, config: Dict[str, Any]|None = None) -> None:
        """
            Loads the raw df from fit file
        """
        self.id = id
        if not config:
        ...
        self.df_raw = load_raw_df_from_fit_v2(path_fit_file, frame_name='record', lat_long_update=True)
    ...
    def get_default_config(self) -> Dict[str, Any]:
        ...
        config["df__elevation_change__gaussian_kernel_sigma"] = 2  # [s], Applies Gaussian kernel to elevation_change_interm column with this standard deviation
    ...
    def preprocess(self, 
        add_experimental_columns = True,
        print_high_grade_warning = False
        ) -> None:
        """
        Preprocesses self.df
        """
        ...
        # Drop these columns since they are not needed anymore (bc. steps_per_min was alreadycomputed)
        if False:
            self.dict_column_imputation_strategy["cadence"] = "drop"
            self.dict_column_imputation_strategy["fractional_cadence"] = "drop"
        ...
        if (not "df__keep_raw_elevation_distance_columns_for_debugging" in self.config.keys()) or self.config["df__keep_raw_elevation_distance_columns_for_debugging"] == False:
        ...
        if not high_grade_rows.empty and print_high_grade_warning:
    ...
    def add_rolling_avg_columns(
        ...
        for base_var in base_vars:
            for i in range(len(window_lengths)):
                ...
                col_name = f"{base_var}_avg_last_{first_previous_time}-{last_previous_time}s"
                ...
                self.df[col_name] = self.df[base_var].shift(last_previous_time).fillna(0).rolling(window=window_size, min_periods=1).apply(helper_pandas.mean_with_zero_imputation, raw=True, kwargs={"expected_length": window_size})
