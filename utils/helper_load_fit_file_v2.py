...
def _add_columns(df, config: Dict[str, Any]):
    """_summary_
    ...
    df["elevation_change"] = (~df["imputed"]).astype(int) * df["elevation_change_raw"] * (df["elevation_change_raw"].abs() <= 1.5)  # assume: sudden elevation change ^= noise
...
@st.cache_data
def load_fit_file(config: Dict[str, Any]) -> pd.DataFrame:
    """
